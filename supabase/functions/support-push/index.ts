...
Deno.serve(async request=>{
  try{
    ...
    const {data:conversation}=await supabase.from('support_conversations').select('customer_name,status').eq('id',message.conversation_id).single()
    if(conversation?.status==='closed')return new Response('closed attendance: notification suppressed',{status:200})
