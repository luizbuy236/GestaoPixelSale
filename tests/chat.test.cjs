...
function setup(){
 ...
 const context={console,URL,Event,confirm:()=>true,queueMicrotask,navigator:{clipboard:{writeText:async text=>sent.push({name:'clipboard',text})}},loadingAdminMessages:false,pendingAdminMessagesLoad:false,pendingAdminMessagesForce:false,lastMessageSignatures:new Map(),supportSearchQuery:'',pendingAdminImage:null,CHAT_EMOJIS:[],CHAT_STICKERS:[],activeChatId:'test',sendingSticker:false,chatConversations:[{id:'test',status:'open',atendimento_started_at:'2026-01-01'}],chatTime:()=>'',decorateSupportMessages(){},showToast(){},setAdminChatStatus(){},startAdminChat(){},loadAdminChats:async()=>{},document:{activeElement:null,createElement:()=>{const n=node();n.addEventListener=(name,fn)=>n[name+'Event']=fn;return n},body:{append:n=>dialogs.push(n)}},$:(selector)=>nodes.get(selector),$$:()=>[],supabaseClient:{rpc:async(name,args)=>{sent.push({name,args});return {data:[],error:null}}}};
 for(const id of ['#supportThread','#supportMessages','#supportReply textarea','#supportReply','#supportImageInput','#supportImagePreview','#supportImageName','#supportImageSize','#sendSupportImage','#cancelSupportImage','#supportEmojiToggle','#supportEmojiPicker','#supportStickerToggle','#supportStickerPicker','#supportStatus','#supportReply button[type="submit"]'])nodes.set(id,node());
 nodes.get('#supportImagePreview').querySelector=()=>nodes.get('#supportImagePreviewImage')||node();
...
test('staff sees an image preview before confirming the upload',async()=>{
 const {context:c,nodes,sent}=setup();c.FileReader=class{readAsDataURL(){this.result='data:image/png;base64,YQ==';this.onload()}};
 const input=nodes.get('#supportImageInput'),file={type:'image/png',size:100,name:'foto.png'};await c.prepareAdminImage(file,input);
 assert.equal(nodes.get('#supportImagePreview').hidden,false);assert.equal(nodes.get('#supportImageName').textContent,'foto.png');assert.equal(sent.some(x=>x.name==='chat_admin_send_image'),false);
 await c.sendAdminImage();
