...
async function loadAdminMessages(scroll=true,force=false){
  ...
  thread.innerHTML=`<header class="support-thread-head"><div><strong>${escapeHtml(conversation.customer_name||'Visitante')}</strong><span>${escapeHtml(conversation.customer_email||'Sem e-mail')}</span></div><div class="support-thread-actions">${!started||conversation.status==='closed'?`<button class="primary" type="button" id="startSupport">${conversation.status==='closed'?'Reabrir atendimento':'Iniciar atendimento'}</button>`:''}<select id="supportStatus"><option value="open">Aberta</option><option value="pending">Pendente</option><option value="closed">Encerrada</option></select></div></header><div class="support-messages" id="supportMessages">${messagesHtml}</div>${started&&conversation.status!=='closed'?`<form class="support-reply" id="supportReply"><div class="support-emoji-wrap"><button class="support-emoji-toggle" id="supportEmojiToggle" type="button" title="Adicionar emoji" aria-label="Adicionar emoji">😊</button><div class="support-emoji-picker" id="supportEmojiPicker" hidden>${CHAT_EMOJIS.map(emoji=>`<button type="button" data-emoji="${emoji}">${emoji}</button>`).join('')}</div></div><div class="support-sticker-wrap"><button class="support-emoji-toggle" id="supportStickerToggle" type="button" title="Enviar figurinha" aria-label="Enviar figurinha">🎴</button><div class="support-sticker-picker" id="supportStickerPicker" hidden>${CHAT_STICKERS.map(([src,label])=>`<button type="button" data-sticker="${src}" title="${label}"><img src="${src}" alt="${label}"></button>`).join('')}</div></div><textarea maxlength="2000" rows="3" aria-label="Mensagem: Enter para enviar; Ctrl+Enter para nova linha" placeholder="Enter: enviar; Ctrl+Enter: nova linha" required></textarea><button class="primary" type="submit">Enviar</button></form>`:'<div class="support-awaiting">'+(conversation.status==='closed'?'Clique em <b>Reabrir atendimento</b> para responder.':'Clique em <b>Iniciar atendimento</b> para responder. O prazo de inatividade de 2 horas começará nesse momento.')+'</div>'}`;
