...
let page='support';
let activeChatId=null,chatPollTimer=null,chatRealtimeChannel=null,chatRealtimeRefreshTimer=null,chatConversations=[],lastChatUnreadTotal=null,lastChatUnreadByConversation=new Map(),chatAudioContext=null,sendingSticker=false,pendingAdminImage=null,loadingAdminChats=false,loadingAdminMessages=false,pendingAdminChatsLoad=false,pendingAdminChatsForce=false,pendingAdminMessagesLoad=false,pendingAdminMessagesForce=false,lastChatListSignature='',lastMessageSignatures=new Map(),supportSearchQuery='';
...
function support(){return `<header class="support-app-header"><div class="support-app-brand"><img src="assets/pixelsale-logo.png" alt="PixelSale"><span></span><div><strong>PixelSale</strong><small>Central de atendimento</small></div></div><div class="support-app-actions"><button class="secondary" type="button" onclick="toggleSupportNoticePanel()">⚠️ Configurar aviso</button><button class="secondary" onclick="enableMobileNotifications()">🔔 Notificações</button><button class="secondary" onclick="enableChatSound()">🔊 Som</button><button class="secondary support-refresh" onclick="loadAdminChats()">↻ Atualizar</button><div class="support-admin"><span class="avatar">AD</span><div><strong>Administrador</strong><small>${escapeHtml(ADMIN_EMAIL)}</small></div></div><button class="support-logout" type="button" onclick="logoutFromSupport()" title="Sair" aria-label="Sair">↪</button></div></header><section class="support-notice-panel" id="supportNoticePanel" hidden><div class="panel-head"><div><h3>Aviso automático no chat</h3><span class="muted">Será enviado ao cliente quando ele abrir o chat durante o período configurado.</span></div><span class="support-notice-state" id="supportNoticeState">Carregando…</span></div><form id="supportNoticeForm"><label class="support-notice-toggle"><input type="checkbox" name="enabled"> Aviso ativado</label><label class="support-notice-message">Mensagem<textarea name="message" maxlength="2000" rows="3" placeholder="Voltarei com o atendimento às 20:00."></textarea></label><label>Início<input type="datetime-local" name="starts_at" required></label><label>Término<input type="datetime-local" name="ends_at" required></label><button class="primary" type="submit">Salvar aviso</button></form><div class="support-notice-preview"><strong>⚠️ Aviso de atendimento</strong><span id="supportNoticePreview">Digite uma mensagem para visualizar.</span></div></section><div class="support-layout"><aside class="support-list-shell"><div class="support-list-heading"><strong>▱ Conversas</strong><span><i></i> Atualizado em tempo real</span><label>⌕<input type="search" id="supportSearch" placeholder="Buscar nome ou mensagem" oninput="filterSupportContacts(this.value)"></label></div><div class="support-list" id="supportList"><div class="support-loading">Carregando conversas…</div></div></aside><section class="support-thread" id="supportThread"><div class="support-placeholder"><span>●</span><strong>Selecione uma conversa</strong><p>As mensagens do cliente aparecerão aqui.</p></div></section></div>`}
function normalizeSupportSearch(value){return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase('pt-BR').trim()}
function filterSupportContacts(value){supportSearchQuery=normalizeSupportSearch(value);let visible=0;$$('.support-contact').forEach(contact=>{const matches=!supportSearchQuery||normalizeSupportSearch(contact.dataset.search||contact.textContent).includes(supportSearchQuery);contact.hidden=!matches;if(matches)visible++});const empty=$('#supportSearchEmpty');if(empty)empty.hidden=visible>0||!supportSearchQuery}
async function logoutFromSupport(){await supabaseClient.auth.signOut();showLogin()}
...
document.addEventListener('pointerdown',()=>unlockChatAudio().catch(()=>false));document.addEventListener('keydown',()=>unlockChatAudio().catch(()=>false));
async function playChatAlert(){let audioReady=false;try{audioReady=await unlockChatAudio()}catch(error){console.warn('Som bloqueado pelo navegador:',error)}if(audioReady){const now=chatAudioContext.currentTime,master=chatAudioContext.createGain(),compressor=chatAudioContext.createDynamicsCompressor();master.gain.value=1;compressor.threshold.value=-18;compressor.knee.value=8;compressor.ratio.value=5;compressor.attack.value=.003;compressor.release.value=.2;master.connect(compressor).connect(chatAudioContext.destination);[0,.28,.56,1.05,1.33,1.61,2.1,2.38,2.66].forEach((delay,index)=>{[760,1040,1380].forEach((frequency,layer)=>{const oscillator=chatAudioContext.createOscillator(),gain=chatAudioContext.createGain();oscillator.type=layer===1?'square':'sawtooth';oscillator.frequency.value=frequency+(index%2?90:0);gain.gain.setValueAtTime(.0001,now+delay);gain.gain.exponentialRampToValueAtTime(.95,now+delay+.012);gain.gain.setValueAtTime(.95,now+delay+.18);gain.gain.exponentialRampToValueAtTime(.0001,now+delay+.27);oscillator.connect(gain).connect(master);oscillator.start(now+delay);oscillator.stop(now+delay+.29)})});setTimeout(()=>{master.disconnect();compressor.disconnect()},3300)}const originalTitle=document.title;document.title='🔴 NOVA MENSAGEM — PixelSale';setTimeout(()=>document.title=originalTitle,7000);if('Notification'in window&&Notification.permission==='granted')new Notification('Nova mensagem no atendimento',{body:'Um cliente enviou uma nova mensagem.',tag:'pixelsale-chat',requireInteraction:true,silent:false});return audioReady}
...
function renderAdminChatList(force=false){const list=$('#supportList');if(!list)return;const signature=chatConversations.map(item=>[item.id,item.updated_at,item.status,item.unread_admin,item.last_message,activeChatId===item.id].join('|')).join(';');if(!force&&signature===lastChatListSignature)return;lastChatListSignature=signature;list.innerHTML=chatConversations.length?chatConversations.map(item=>{const search=[item.customer_name,item.customer_email,item.last_message].filter(Boolean).join(' ');return `<button class="support-contact ${activeChatId===item.id?'active':''}" data-search="${escapeHtml(search)}" onclick="selectAdminChat('${item.id}')"><span class="support-avatar">${escapeHtml((item.customer_name||'V').slice(0,2).toUpperCase())}</span><span class="support-contact-copy"><strong>${escapeHtml(item.customer_name||'Visitante')}</strong><small>${escapeHtml(item.last_message||'Nova conversa')}</small><time>${chatTime(item.updated_at)}</time></span>${Number(item.unread_admin)>0?`<b>${item.unread_admin}</b>`:''}</button>`}).join('')+'<div class="support-loading support-search-empty" id="supportSearchEmpty" hidden>Nenhuma conversa encontrada.</div>':'<div class="support-loading">Nenhuma conversa ainda.</div>';filterSupportContacts(supportSearchQuery)}
...
function renderSupportText(value){
  const text=String(value||''),pattern=/(?:https?:\/\/|www\.)[^\s<>"']+/gi;
  let html='',offset=0;
  for(const match of text.matchAll(pattern)){
    const link=match[0].replace(/[.,!?;:]+$/,'');
    html+=escapeHtml(text.slice(offset,match.index));
    const href=/^www\./i.test(link)?'https://'+link:link;
    html+='<a href="'+escapeHtml(href)+'" target="_blank" rel="noopener noreferrer">'+escapeHtml(link)+'</a>';
    offset=match.index+link.length;
  }
  return html+escapeHtml(text.slice(offset));
}
function renderSupportMessage(message){
  const media=String(message.media_data||'');
...
function bindSupportImages(container=$('#supportMessages')){
  container?.querySelectorAll('.support-image-open').forEach(button=>button.onclick=()=>{
    const dialog=document.createElement('dialog');dialog.className='support-image-viewer';
    const close=document.createElement('button');close.type='button';close.textContent='Fechar';close.className='secondary';
    const img=document.createElement('img');img.src=button.querySelector('img').src;img.alt='Imagem ampliada do chat';
    dialog.append(close,img);document.body.append(dialog);close.onclick=()=>dialog.close();
    dialog.addEventListener('close',()=>dialog.remove());dialog.showModal();
  });
}
function bindSupportMessageActions(container=$('#supportMessages')){
  if(!container)return;
  container.querySelectorAll('[data-copy-message]').forEach(button=>button.onclick=async()=>{try{await navigator.clipboard.writeText(button.dataset.copyMessage);showToast('Mensagem copiada.')}catch(error){console.error(error);showToast('Não foi possível copiar a mensagem.')}});
...
async function loadAdminMessages(scroll=true,force=false){
  ...
  const currentMessages=$('#supportMessages');
  if(!scroll&&currentMessages){const keepAtBottom=currentMessages.scrollHeight-currentMessages.scrollTop-currentMessages.clientHeight<80;currentMessages.innerHTML=messagesHtml;decorateSupportMessages(currentMessages);bindSupportImages(currentMessages);bindSupportMessageActions(currentMessages);if(keepAtBottom)currentMessages.scrollTop=currentMessages.scrollHeight;return}
  thread.innerHTML=`<header class="support-thread-head"><div><strong>${escapeHtml(conversation.customer_name||'Visitante')}</strong><span>${escapeHtml(conversation.customer_email||'Sem e-mail')}</span></div><div class="support-thread-actions">${conversation.status==='closed'?'<button class="primary" type="button" id="startSupport">Reabrir atendimento</button>':''}<select id="supportStatus"><option value="open">Aberta</option><option value="pending">Pendente</option><option value="closed">Encerrada</option></select></div></header><div class="support-messages" id="supportMessages">${messagesHtml}</div>${conversation.status!=='closed'?`<form class="support-reply" id="supportReply"><div class="support-reply-title"><strong>Responder ao cliente</strong><span>Enter envia · Ctrl+Enter cria uma nova linha</span></div><div class="support-image-preview" id="supportImagePreview" hidden><img alt="Prévia da imagem selecionada"><div><strong id="supportImageName"></strong><span id="supportImageSize"></span></div><button type="button" class="secondary" id="cancelSupportImage">Cancelar</button><button type="button" class="primary" id="sendSupportImage">Enviar imagem</button></div><label class="support-image-upload" title="Selecionar imagem"><span>📎</span><span class="support-image-upload-text">Imagem</span><input id="supportImageInput" type="file" accept="image/jpeg,image/png,image/webp" aria-label="Selecionar imagem para enviar"></label><div class="support-emoji-wrap"><button class="support-emoji-toggle" id="supportEmojiToggle" type="button" title="Adicionar emoji" aria-label="Adicionar emoji">😊</button><div class="support-emoji-picker" id="supportEmojiPicker" hidden>${CHAT_EMOJIS.map(emoji=>`<button type="button" data-emoji="${emoji}">${emoji}</button>`).join('')}</div></div><div class="support-sticker-wrap"><button class="support-emoji-toggle" id="supportStickerToggle" type="button" title="Enviar figurinha" aria-label="Enviar figurinha">🎴</button><div class="support-sticker-picker" id="supportStickerPicker" hidden>${CHAT_STICKERS.map(([src,label])=>`<button type="button" data-sticker="${src}" title="${label}"><img src="${src}" alt="${label}"></button>`).join('')}</div></div><textarea maxlength="2000" rows="3" aria-label="Mensagem: Enter para enviar; Ctrl+Enter para nova linha" placeholder="Escreva sua mensagem para o cliente…" required></textarea><button class="primary" type="submit">Enviar mensagem</button></form>`:'<div class="support-awaiting">Clique em <b>Reabrir atendimento</b> para responder.</div>'}`;
  decorateSupportMessages();bindSupportImages();bindSupportMessageActions();const nextInput=$('#supportReply textarea');if(nextInput){nextInput.value=draft;if(restoreFocus){nextInput.focus();if(selectionStart!=null)nextInput.setSelectionRange(selectionStart,selectionEnd)}nextInput.onkeydown=event=>{if(event.key!=='Enter'||event.isComposing)return;if(event.ctrlKey){event.preventDefault();const start=nextInput.selectionStart,end=nextInput.selectionEnd;if(nextInput.value.length-(end-start)<nextInput.maxLength){nextInput.setRangeText('\n',start,end,'end');nextInput.dispatchEvent(new Event('input',{bubbles:true}))}}else if(!event.shiftKey&&!event.altKey&&!event.metaKey){event.preventDefault();$('#supportReply').requestSubmit()}};$('#supportReply').onsubmit=sendAdminReply;$('#supportImageInput').onchange=event=>prepareAdminImage(event.target.files?.[0],event.target);$('#cancelSupportImage').onclick=clearAdminImagePreview;$('#sendSupportImage').onclick=sendAdminImage;$('#supportEmojiToggle').onclick=()=>{$('#supportEmojiPicker').hidden=!$('#supportEmojiPicker').hidden;$('#supportStickerPicker').hidden=true};$$('#supportEmojiPicker [data-emoji]').forEach(button=>button.onclick=()=>insertChatEmoji(button.dataset.emoji));$('#supportStickerToggle').onclick=()=>{$('#supportStickerPicker').hidden=!$('#supportStickerPicker').hidden;$('#supportEmojiPicker').hidden=true};$$('#supportStickerPicker [data-sticker]').forEach(button=>button.onclick=()=>sendAdminSticker(button.dataset.sticker,button))}if($('#startSupport'))$('#startSupport').onclick=startAdminChat;$('#supportStatus').value=conversation.status;$('#supportStatus').onchange=event=>setAdminChatStatus(event.target.value);if(scroll){const box=$('#supportMessages');box.scrollTop=box.scrollHeight}
}
...
function clearAdminImagePreview(){pendingAdminImage=null;const preview=$('#supportImagePreview'),input=$('#supportImageInput');if(preview)preview.hidden=true;if(input)input.value=''}
async function prepareAdminImage(file,input){if(!file||sendingSticker)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type)){input.value='';return showToast('Escolha uma imagem JPG, PNG ou WebP.')}if(file.size>10000000){input.value='';return showToast('A imagem deve ter no máximo 10 MB.')}try{const mediaData=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file)});pendingAdminImage={mediaData,name:file.name||'Imagem',size:file.size};const preview=$('#supportImagePreview');preview.querySelector('img').src=mediaData;$('#supportImageName').textContent=pendingAdminImage.name;$('#supportImageSize').textContent=(file.size/1048576).toLocaleString('pt-BR',{maximumFractionDigits:1})+' MB';preview.hidden=false}catch(error){console.error(error);input.value='';showToast('Não foi possível carregar a prévia da imagem.')}}
async function sendAdminImage(){if(!pendingAdminImage||sendingSticker)return;const button=$('#sendSupportImage'),mediaData=pendingAdminImage.mediaData;sendingSticker=true;button.disabled=true;try{const {error}=await supabaseClient.rpc('chat_admin_send_image',{p_conversation_id:activeChatId,p_media_data:mediaData});if(error)throw error;clearAdminImagePreview();showToast('Imagem enviada.')}catch(error){console.error(error);showToast(error.message||'Não foi possível enviar a imagem.')}finally{sendingSticker=false;button.disabled=false}await loadAdminMessages(false,true);loadAdminChats()}
