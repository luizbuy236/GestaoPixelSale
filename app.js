...
function support(){return head('Atendimento','Converse em tempo real com os clientes da loja',`<button class="secondary" onclick="loadAdminChats()">↻ Atualizar</button>`)+`<div class="support-layout"><aside class="support-list" id="supportList"><div class="support-loading">Carregando conversas…</div></aside><section class="support-thread" id="supportThread"><div class="support-placeholder"><span>●</span><strong>Selecione uma conversa</strong><p>As mensagens do cliente aparecerão aqui.</p></div></section></div>`}
const chatTime=value=>new Date(value).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});
function unlockChatAudio(){if(!chatAudioContext)chatAudioContext=new (window.AudioContext||window.webkitAudioContext)();if(chatAudioContext.state==='suspended')chatAudioContext.resume()}
document.addEventListener('pointerdown',unlockChatAudio,{once:true});document.addEventListener('keydown',unlockChatAudio,{once:true});
function playChatAlert(){unlockChatAudio();if(!chatAudioContext)return;const now=chatAudioContext.currentTime;[0,.22,.44,.78].forEach((delay,index)=>{const oscillator=chatAudioContext.createOscillator(),gain=chatAudioContext.createGain();oscillator.type='square';oscillator.frequency.value=index===3?1320:980;gain.gain.setValueAtTime(.0001,now+delay);gain.gain.exponentialRampToValueAtTime(.42,now+delay+.015);gain.gain.exponentialRampToValueAtTime(.0001,now+delay+.16);oscillator.connect(gain).connect(chatAudioContext.destination);oscillator.start(now+delay);oscillator.stop(now+delay+.18)});const originalTitle=document.title;document.title='🔴 NOVA MENSAGEM — PixelSale';setTimeout(()=>document.title=originalTitle,5000);if('Notification'in window&&Notification.permission==='granted')new Notification('Nova mensagem no atendimento',{body:'Um cliente enviou uma nova mensagem.',tag:'pixelsale-chat'})}
function updateChatUnreadAlert(rows){const unreadTotal=(rows||[]).reduce((total,item)=>total+Number(item.unread_admin||0),0);if(lastChatUnreadTotal!==null&&unreadTotal>lastChatUnreadTotal)playChatAlert();lastChatUnreadTotal=unreadTotal}
...
async function loadAdminMessages(scroll=true){
  ...
  const {data,error}=await supabaseClient.rpc('chat_admin_messages',{p_conversation_id:activeChatId});if(error)return showToast('Não foi possível carregar as mensagens.');
  thread.innerHTML=`<header class="support-thread-head"><div><strong>${escapeHtml(conversation.customer_name||'Visitante')}</strong><span>${escapeHtml(conversation.customer_email||'Sem e-mail')}</span></div><select id="supportStatus"><option value="open">Aberta</option><option value="pending">Pendente</option><option value="closed">Encerrada</option></select></header><div class="support-messages" id="supportMessages">${(data||[]).map(message=>`<div class="support-message ${message.sender==='admin'?'support-message-admin':''} ${message.message_type==='image'?'support-message-image':''}">${message.message_type==='image'&&message.media_data?`<a href="${message.media_data}" target="_blank" rel="noopener"><img src="${message.media_data}" alt="Foto enviada pelo cliente"></a>`:escapeHtml(message.body)}<time>${chatTime(message.created_at)}</time></div>`).join('')||'<div class="support-loading">Nenhuma mensagem.</div>'}</div><form class="support-reply" id="supportReply"><textarea maxlength="2000" rows="1" placeholder="Digite sua resposta…" required></textarea><button class="primary" type="submit">Enviar</button></form>`;
  const nextInput=$('#supportReply textarea');nextInput.value=draft;if(restoreFocus){nextInput.focus();if(selectionStart!=null)nextInput.setSelectionRange(selectionStart,selectionEnd)}$('#supportStatus').value=conversation.status;$('#supportStatus').onchange=event=>setAdminChatStatus(event.target.value);$('#supportReply').onsubmit=sendAdminReply;if(scroll){const box=$('#supportMessages');box.scrollTop=box.scrollHeight}
}
async function sendAdminReply(event){event.preventDefault();const input=$('#supportReply textarea'),body=input.value.trim();if(!body)return;const button=$('#supportReply button');button.disabled=true;const {error}=await supabaseClient.rpc('chat_admin_send',{p_conversation_id:activeChatId,p_body:body});button.disabled=false;if(error)return showToast('Não foi possível enviar a resposta.');input.value='';await loadAdminChats();await loadAdminMessages(true)}
