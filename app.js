...
function finance(){
  const fees=db.expenses.filter(e=>e.category==='Mensalidade da loja').sort((a,b)=>b.date.localeCompare(a.date));
  const feeTotal=sumRows(fees),currentFeeTotal=sumRows(fees.filter(e=>String(e.date).slice(0,7)===today.slice(0,7)));
  const feeTable=`<div class="panel table-panel"><div class="panel-head"><div><h3>Mensalidades da loja</h3><span class="muted">Custos mensais da operação</span></div><button class="primary" onclick="openForm('storeFee')">＋ Adicionar mensalidade</button></div><table><thead><tr><th>Referência</th><th>Valor</th><th>Pagamento</th><th>Vencimento / pagamento</th><th></th></tr></thead><tbody>${fees.map(f=>`<tr><td><b>${escapeHtml(f.description)}</b>${f.notes?`<br><span class="muted">${escapeHtml(f.notes)}</span>`:''}</td><td>${money(f.value)}</td><td>${escapeHtml(f.payment||'—')}</td><td>${date(f.date)}</td><td>${rowActions('storeFee','expenses',f.id)}</td></tr>`).join('')||`<tr><td colspan="5" class="empty"><b>Nenhuma mensalidade cadastrada</b>Adicione a mensalidade da loja para incluí-la nos custos e relatórios.</td></tr>`}</tbody></table></div>`;
  const expenses=genericPage('expenses','Controle financeiro','Receitas e despesas em um só lugar',[['Descrição','description'],['Categoria','category'],['Valor','value','money'],['Pagamento','payment'],['Data','date','date']],'expense');
