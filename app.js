...
async function sendAdminImage(file,input){if(!file||sendingSticker)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type))return showToast('Escolha uma imagem JPG, PNG ou WebP.');if(file.size>10000000){input.value='';return showToast('A imagem deve ter no máximo 10 MB.')}sendingSticker=true;input.disabled=true;try{const mediaData=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file)});const {error}=await supabaseClient.rpc('chat_admin_send_image',{p_conversation_id:activeChatId,p_media_data:mediaData});if(error)throw error;showToast('Imagem enviada.')}catch(error){console.error(error);showToast(error.message||'Não foi possível enviar a imagem.')}finally{sendingSticker=false;input.disabled=false;input.value=''}await loadAdminMessages(false,true);loadAdminChats()}
