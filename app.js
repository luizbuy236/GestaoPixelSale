...
async function playChatAlert(){if(!await unlockChatAudio())return false;const now=chatAudioContext.currentTime;[0,.2,.4,.8,1,1.2,1.6,1.8].forEach((delay,index)=>{const oscillator=chatAudioContext.createOscillator(),gain=chatAudioContext.createGain();oscillator.type=index%2?'square':'sawtooth';oscillator.frequency.value=index%2?1180:880;gain.gain.setValueAtTime(.0001,now+delay);gain.gain.exponentialRampToValueAtTime(.75,now+delay+.012);gain.gain.exponentialRampToValueAtTime(.0001,now+delay+.18);oscillator.connect(gain).connect(chatAudioContext.destination);oscillator.start(now+delay);oscillator.stop(now+delay+.2)});const originalTitle=document.title;document.title='🔴 NOVA MENSAGEM — PixelSale';setTimeout(()=>document.title=originalTitle,7000);if('Notification'in window&&Notification.permission==='granted')new Notification('Nova mensagem no atendimento',{body:'Um cliente enviou uma nova mensagem.',tag:'pixelsale-chat',requireInteraction:true});return true}
