...
function renderSupportMessage(message){
  ...
  const content=src?'<button type="button" class="support-image-open" aria-label="Ampliar imagem"><img src="'+escapeHtml(src)+'" alt="Imagem enviada no chat"></button>':'<div class="support-message-text">'+renderSupportText(message.body)+'</div>';
  return '<div class="support-message '+(message.sender==='admin'?'support-message-admin':'')+' '+(src?'support-message-image':'')+'">'+content+'<time>'+chatTime(message.created_at)+'</time></div>';
}
...
async function loadAdminMessages(scroll=true){
  ...
  const requestedChatId=activeChatId,conversation=chatConversations.find(item=>item.id===requestedChatId);if(!conversation)return;
  ...
  const {data,error}=await supabaseClient.rpc('chat_admin_messages',{p_conversation_id:requestedChatId});if(error)return showToast('Não foi possível carregar as mensagens.');if(activeChatId!==requestedChatId)return;
...
async function sendAdminReply(event){
  ...
  input.value='';await loadAdminChats();await loadAdminMessages(true)
}
...
function render(){clearInterval(chatPollTimer);renderNav();$('#content').innerHTML=renderers[page]();if(page==='sales'){const filter=()=>{let q=$('#saleSearch').value.toLowerCase(),st=$('#saleStatus').value,cat=$('#saleCategory').value;$('#salesResult').innerHTML=salesTable(db.sales.filter(s=>(s.client+s.product+(s.contact||'')).toLowerCase().includes(q)&&(!st||s.status===st)&&(!cat||s.category===cat)))};['saleSearch','saleStatus','saleCategory'].forEach(id=>$('#'+id).oninput=filter)}const poll=page==='support'?loadAdminChats:pollChatNotifications;poll();chatPollTimer=setInterval(poll,page==='support'?1000:3000);updateNotifications()}
const forms={
