...
let page='dashboard';
let activeChatId=null,chatPollTimer=null,chatConversations=[];
...
function render(){clearInterval(chatPollTimer);renderNav();$('#content').innerHTML=renderers[page]();if(page==='sales'){const filter=()=>{let q=$('#saleSearch').value.toLowerCase(),st=$('#saleStatus').value,cat=$('#saleCategory').value;$('#salesResult').innerHTML=salesTable(db.sales.filter(s=>(s.client+s.product+(s.contact||'')).toLowerCase().includes(q)&&(!st||s.status===st)&&(!cat||s.category===cat)))};['saleSearch','saleStatus','saleCategory'].forEach(id=>$('#'+id).oninput=filter)}if(page==='support'){loadAdminChats();chatPollTimer=setInterval(loadAdminChats,3000)}updateNotifications()}
const forms={
